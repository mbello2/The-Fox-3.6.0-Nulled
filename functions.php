...
// Thefox custom posts HOOKS
function rd_check_tcp_status() {
include_once(ABSPATH.'wp-admin/includes/plugin.php');
if(is_plugin_active('thefox_cp/thefox_custom_post.php')) {
 return true;
...
//seperate style file in two for IE style limit.
		...
		wp_enqueue_style( 'googlefonts', "$protocol://fonts.googleapis.com/css?family=Lato:100,300,400,600,700,900|Open+Sans:700,300,600,400|Raleway:900|Playfair+Display|" );
...
function rd_scripts(){
		...
		wp_enqueue_script( 'scrollmagic', RD_DIRECTORY. '/js/scrollmagic.js', array( 'jquery' ), false, true );
...
function registerBackendJavascript() {

}

if( rd_check_woo_status() == true) {
if(!function_exists( 'thefox_woo_functions' )){
function thefox_woo_functions($type){
if($type == 'classic'){
...
if(isset($rd_data['rd_shop_design'])){
thefox_woo_functions($rd_data['rd_shop_design']);
}
...
// Create share icons
include("single_demo_import/init.php");
...
if(!function_exists( 'thefox_vc_functions' )){

function thefox_vc_functions(){
...
function rd_register_required_plugins() {
	$plugins = array(
		array(
			...
			'source'   				=> get_template_directory(). '/includes/plugins/thefox_cp.zip', // The plugin source
...
function rd_register_menu() {
	...
	register_nav_menu('one-page-mobile-menu', __('One page Mobile Menu','thefoxwp'));
}
